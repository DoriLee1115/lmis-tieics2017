...
public class BidRestController {
    ...
    public ResponseEntity<Void> create(Bid bid, UriComponentsBuilder ucBuilder) {
    ...
    public ResponseEntity<Bid> update(@PathVariable("id") Integer id, Bid bid) {
        Bid oldBid = bidService.findById(id);
        if (oldBid==null) {
            return new ResponseEntity<Bid>(HttpStatus.NOT_FOUND);
        }
        if(bid.getId() != null)
        	oldBid.setId(bid.getId()); 
        if(bid.getWaybillId() != null)
        	oldBid.setWaybillId(bid.getWaybillId()); 
        if(bid.getCarrierId() != null)
